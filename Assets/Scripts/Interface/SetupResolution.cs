...
public class SetupResolution : MonoBehaviour {
	...
	void ReduceResolution(){
		...
		if (newResIndex != resIndex){
			...
			GoogleAnalytics.Client.SendTimedEventHit("reduceResolution",  Level.instance.name, (resolutions[newResIndex].width + "x" + resolutions[newResIndex].height), Time.fixedTime);
//						
